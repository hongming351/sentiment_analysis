...
# ==================== 训练单折SVM ====================
def train_svm_fold(fold_idx, train_df, val_df, config):
    """训练单个SVM模型"""
    ...
    val_labels = val_df[config['label_column']].tolist()
...
def main():
    ...
    # 计算项目根目录（svm_model的上一级是models，再上一级是项目根）
    ...
    # 保存配置
    with open(os.path.join(results_dir, 'svm_config.json'), 'w', encoding='utf-8') as f:
    ...
    with open(os.path.join(results_dir, 'svm_training_summary.json'), 'w', encoding='utf-8') as f:
    ...
    print(f"  生成的模型: svm_models/svm_fold_0.pkl 到 svm_fold_4.pkl")
    
    print(f"\n🚀 下一步:")
    print(f"  运行 evaluate_svm.py 评估模型性能")
    print(f"  运行 python evaluate_svm.py")
